...
class DPC(object):
    ...
    def __init__(self, path, data_name, n=0, dc_method=0, dc_percent=1, rho_method=1, delta_method=1, use_halo=False, plot=None):
        ...
        for i in range(len(center)):
            center_true[i] = pending_points[center[i]]
        ...
        if plot is None:
            ...
            axes[1].set_title('rho_threshold-' + str(rho_threshold))
            self.draw_roh_delta(rho_new, delta, center, axes[0])
            self.draw_gamma(rho_new, delta, axes[1])
    ...
    def del_points(self, points, rho, rho_threshold):
        # print('1')
        mean_rho = sorted(rho)[int(len(rho) * rho_threshold)]
        # print(mean_rho)
        print('rho:', rho)
        new_points_list = []
        pending_points = []
        ...
        for i in range(len(rho)):
            if rho[i] >= mean_rho:
                pending_points.append(i)

                # print(points[i])
                new_points_list.append([points.iat[i, 0], points.iat[i, 1]])
        new_points = pd.DataFrame(new_points_list, columns=['x', 'y'])
        max_id = len(new_points)
        # print(max_id)
        d = pd.DataFrame(np.zeros((max_id, max_id)))  # 距离矩阵
        dis = sch.distance.pdist(new_points, 'euclidean')  # 欧式距离
        n = 0
        for i in range(max_id):
            for j in range(i + 1, max_id):
                d.at[i, j] = dis[n]
        ...
        min_dis = d.min().min()
        max_dis = d.max().max()
        return new_points, d, dis, min_dis, max_dis, max_id, pending_points
